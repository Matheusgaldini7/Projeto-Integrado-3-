...
class _RefeitorioScreenState extends State<RefeitorioScreen> {
  ...
  void initState() {
    ...
    _storyText = '${_player.nome} chega ao Refeitório da PUC-Campinas.\n\n'
  ...
  void _explore() => _set(() {
        _storyText = '${_player.nome} observa melhor o Refeitório.\n\n'
            'Ao fundo, Nutri prepara cafés atrás do balcão. Próximo a uma mesa, um Veterano mexe em uma mochila cheia de cabos e livros.';
        _mode = 'mainChoice';
      });

  void _talkNutri() => _set(() {
        _storyText = 'Nutri: "${_player.titulo} ${_player.nome}, você parece exausto. Aqui é uma Zona Segura.\n\n'
  ...
  void _talkVeterano() => _set(() {
        _storyText = 'Veterano: "${_player.nome}, se você quer sobreviver ao campus, não basta estudar. Tem que saber a hora certa de parar, recuperar energia e voltar mais forte."';
  ...
  void _prepare() => _set(() {
        ...
        if (!_cafeComprado && !inv.contains('Café Energético')) { inv.add('Café Energético'); _cafeComprado = true; }
  ...
  void _goNext() => Navigator.pushReplacement(context, MaterialPageRoute(
      builder: (_) => widget.voltouDeH15 ? PolitecnicaScreen(player: _player) : H06Screen(player: _player)));
          ...
          child: Column(children: [
              ...
              child: Padding(padding: const EdgeInsets.all(12),
                child: Column(children: [
                  Text('STATUS DE ${_player.nome.toUpperCase()}',
                      style: const TextStyle(color: Color(0xFFF59E0B), fontWeight: FontWeight.bold, letterSpacing: 1.5)),
            ...
            Expanded(
              child: EnvironmentCard(
                text: _storyText,
                image: 'assets/backgrounds/refeitorio.png',
              ),
            ),
            const SizedBox(height: 10),
            _buildActions(),
  ...
  Widget _hpBar(String label, int hp, int max) =>
      Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('$label: $hp/$max', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13)),
        const SizedBox(height: 4),
        ClipRRect(borderRadius: BorderRadius.circular(8),
          child: LinearProgressIndicator(value: (hp / max).clamp(0.0, 1.0), minHeight: 10,
  ...
  Widget _buildActions() {
    ...
    if (_mode == 'readyToLeave') return Column(children: [
      _btn(widget.voltouDeH15 ? 'Ir para a Politécnica' : 'Ir para o H06', Icons.arrow_forward, _goNext),

...
class _PolitecnicaScreenState extends State<PolitecnicaScreen> {
  ...
  void _talkEngineer() => _set(() {
        _storyText = 'Engenheiro: "Os cálculos são traiçoeiros. Você pode tentar resolver um enigma agora. Se acertar, terá vantagem contra O Derivador."';
        _mode = 'engineerChoice';
      });

  void _tryPuzzle() => _set(() {
        _storyText = '${_player.titulo} ${_player.nome}: "Quero tentar"\n\nO terminal exibe:\n"Se uma máquina produz 4 peças por minuto, quantas peças ela produz em 5 minutos?"';
        _mode = 'puzzle';
      });
  ...
  void _answerPuzzle(int i) => _set(() {
        ...
        _storyText = _puzzleBonus ? 'Você acertou! O Derivador começará enfraquecido.' : 'Você errou. Seguirá sem vantagem.';
        _mode = 'beforeBoss';
      });
  ...
  void _meetBoss() => _set(() {
        _storyText = 'O Derivador: "Você superou a primeira avaliação, mas agora está diante da verdadeira barreira matemática.\n\nEscolha como deseja ser avaliado."';
        _mode = 'choice';
      });
  ...
  void _attack() => _set(() {
        ...
        if (_bossHp <= 0) { _mode = 'reward'; _storyText += '\n\nO Derivador: "Sua solução foi... elegante."'; return; }
  ...
  void _goNext() => Navigator.pushReplacement(context,
      MaterialPageRoute(builder: (_) => RefeitorioScreen(player: _player)));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Politécnica — Laboratório da Lógica', style: TextStyle(fontWeight: FontWeight.bold)),
        centerTitle: true,
          ...
          child: Column(children: [
            ...
            Expanded(
              child: EnvironmentCard(
                text: _storyText,
                image: 'assets/backgrounds/ct.png',
  ),
),

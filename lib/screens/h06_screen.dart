...
class _H06ScreenState extends State<H06Screen> {
  ...
  final List<Map<String, dynamic>> _questions = [
    {'question': 'Para que serve uma variável?',
     'options': ['Armazenar dados durante a execução', 'Desligar o computador', 'Criar imagens automaticamente'], 'answer': 0},
    {'question': 'O que é uma função?',
     'options': ['Um bloco de código que executa uma tarefa', 'Uma falha do sistema', 'Um tipo de monitor'], 'answer': 0},
    {'question': 'O que significa erro de sintaxe?',
     'options': ['Erro na escrita do código', 'Erro causado pela internet', 'Problema no teclado'], 'answer': 0},
    {'question': 'O que é clean code?',
     'options': ['Código limpo, legível e bem organizado', 'Código sem comentários', 'Código que apaga arquivos'], 'answer': 0},
    {'question': 'Função de um comentário no código?',
     'options': ['Explicar o que o código faz', 'Executar uma instrução extra', 'Diminuir o arquivo'], 'answer': 0},
  ];
  ...
  void initState() {
    ...
    _storyText = '${_player.nome} chega ao prédio H06.\n\n'
  ...
  void _explore() => _set(() {
        _storyText = '${_player.nome} avança pelo laboratório do H06.\n\n'
  ...
  void _readCode() => _set(() {
        ...
        _storyText = '${_player.titulo} ${_player.nome}: "Claro, deixa eu ver seu código."\n\n'
  ...
  void _attack() => _set(() {
        ...
        if (_bossHp <= 0) { _mode = 'reward'; _storyText += '\n\nO Compilador: "Build: SUCCESS. Você passou na revisão."'; return; }
  ...
  void _receiveReward() {
    ...
    _player = _player.copyWith(inventario: inv, skills: sk,
        fasesVencidas: _player.fasesVencidas + 1, hp: _player.hpMax + 20);
    _set(() {
      _rewardReceived = true;
      _storyText = 'Recompensa recebida!\n\n🎁 Item: IDE\n✨ Skill: Clean Code\n'
  ...
  void _goNext() => Navigator.pushReplacement(context,
      MaterialPageRoute(builder: (_) => AuditorioScreen(player: _player)));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('H06 — Laboratório de Programação', style: TextStyle(fontWeight: FontWeight.bold)),
        centerTitle: true,
          ...
          child: Column(children: [
            ...
            Expanded(
              child: EnvironmentCard(
                text: _storyText,
                image: 'assets/backgrounds/h06.png',
              ),
            ),
  ...
  Widget _buildActions() {
    ...
    if (_mode == 'battle') return BattleActions(player: _player, battle: _battle,
        onAtacar: _attack, onUsarItem: _usarItem, onFugir: () => Navigator.pop(context));
  ...
  Widget _btn(String t, IconData i, VoidCallback fn) => Padding(
        ...
        child: SizedBox(width: double.infinity, height: 46,
          child: ElevatedButton.icon(onPressed: fn, icon: Icon(i),

...
class _MiniMapaState extends State<MiniMapa>
  ...
  Widget build(BuildContext context) {
    final destino = ambientesPUC.firstWhere((a) => a.id == widget.ambienteAlvo);

    return Align(
      alignment: widget.compacto ? Alignment.center : Alignment.centerRight,
      child: Container(
        width: widget.compacto ? double.infinity : 110,
        padding: EdgeInsets.all(widget.compacto ? 6 : 8),
        ...
        child: widget.compacto
            ? _buildCompact(destino)
            : _buildDefault(destino),
      ),
    );
  }

  Widget _buildDefault(Ambiente destino) {
    return Column(
      ...
      children: [
        ...
        if (_carregando)
          const SizedBox(
            height: 70,
            child: Center(child: _LoadingGps()),
          )
        else if (_erro != null)
          SizedBox(height: 70, child: _buildErro())
        else if (_pos != null) ...[
  ...
  Widget _buildCompact(Ambiente destino) {
    return Column(
      children: [
        _buildHeader(destino),
            ...
            child: _carregando
                ...
                : _erro != null
                    ...
                    : _pos == null
                        ...
                        : FittedBox(
                            fit: BoxFit.contain,
                            child: _buildRadar(_pos!, destino),
                          ),
          ),
        ),
      ],
  ...
  Widget _buildHeader(Ambiente destino) {
    return Row(
      ...
      children: [
        ...
        GestureDetector(
          onTap: _atualizar,
          child: const Icon(Icons.refresh, color: Color(0xFF38BDF8), size: 12),
        ),
      ],
    );
  }
      ...
      child: AnimatedBuilder(
        ...
        builder: (_, __) => Stack(
          ...
          children: [
            ...
            if (!dentroRaio)
              Transform.translate(
                offset: Offset(math.sin(angulo) * 24, -math.cos(angulo) * 24),
                child: Container(
                  width: 6,
                  height: 6,
                  decoration: BoxDecoration(
  ...
  Widget _buildInfo(Position pos, Ambiente destino) {
    ...
    final dir = _formatDirection(graus);
    final distStr =
  ...
  String _formatDirection(double graus) {
    if (graus < 22.5 || graus >= 337.5) return 'N';
...
class _LoadingGps extends StatelessWidget {
  const _LoadingGps();
...
class _RadarPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final cor =
        dentroRaio ? const Color(0xFF48D058) : const Color(0xFF38BDF8);
